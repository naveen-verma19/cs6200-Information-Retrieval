...
common_url_stop_words = ["cookie", "books.google", "video", "contact-us", "donate", "pubmed", "terms-of-use",
                         "privacy-policy", "email", "javascript"]

wiki_stop_words = wiki_stop_words + common_url_stop_words
wiki_stop_words = set(list(map(lambda x: x.lower(), wiki_stop_words)))
...
def get_exact_outgoing_links(soup_obj, base_url, rules, whitelist_domains):
    li = []
    counted_hrefs = set()
    all_link_objs = soup_obj.find_all('a')
    print("fetched all links from soup")
    for link in all_link_objs:
        href = link.get("href")
        if not href:
            continue
        href = href.strip()
        ...
        concat_link_string = href
        if title:
            concat_link_string += concat_link_string

        link_text = link.string
        if link_text:
            concat_link_string+=link_text

        if not isgoodText(concat_link_string, domain):
            continue
